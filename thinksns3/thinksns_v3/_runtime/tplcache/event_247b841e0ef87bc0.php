...
<div class="content no_bg boxShadow">
  <!-- 内容 begin  -->
<div class="find-type" style="height:26px;">
	<div class="app-title">
		<div class="search-input" style="z-index:99;">
            <div id="mod-search" model-node="drop_weiba_search">
                <form name="search_event" id="search_event" method="post" action="<?php echo U('event/Index/index');?>">
                    <dl>
                    <dt class="clearfix"><!--<i class="ico-search"></i>--><input autocomplete="off" id="searchevent_input" name="title" class="s-txt left"  type="text" value="<?php if(!empty($searchkey)){ ?><?php echo ($searchkey); ?><?php }else{ ?>搜活动<?php } ?>" onfocus="this.value=''" onblur="setTimeout(function(){ $('#search-box').remove();} , 200);if(this.value=='') this.value='搜活动';" event-node="searchKey" name='k'><a href="javascript:void(0)" class="btn-search" onclick="if(getLength($('#searchevent_input').val()) && $('#searchevent_input').val()!=='搜活动'){ $('#search_event').submit(); return false;}"><span>搜索</span></a></dt>
                    </dl>
                </form>
            </div>
      </div>
		<h4 class="left"><img src="<?php echo ($ts['app']['icon_url']); ?>" /><?php echo ($ts['app']['app_alias']); ?></h4>
		<div class="app-tab-menu clearfix" style="margin:0 0 0 130px;">
			<ul>
				<li><a href="<?php echo U('event/Index/index');?>" <?php if(!in_array((ACTION_NAME),array('personal','addEvent','edit','eventDetail')))echo 'class="current"'; ?> <?php if(in_array((ACTION_NAME),array('eventDetail'))&& $uid!=$mid)echo 'class="current"'; ?>><?php echo ($ts['app']['app_alias']); ?>首页
          <?php if(!in_array((ACTION_NAME),array('personal','addEvent','edit','eventDetail'))){ ?><span class="triangle"></span><?php } ?>
          <?php if(in_array((ACTION_NAME),array('eventDetail'))&& $uid!=$mid){ ?><span class="triangle"></span><?php } ?>
        </a><i class="line"></i></li>

        <li><a href="<?php echo U('event/Index/personal', array('action'=>'launch'));?>" <?php if(in_array((ACTION_NAME),array('personal','addEvent','edit')))echo 'class="current"'; ?> <?php if(in_array((ACTION_NAME),array('eventDetail'))&& $uid==$mid)echo 'class="current"'; ?>>我的<?php echo ($ts['app']['app_alias']); ?>
          <?php if(in_array((ACTION_NAME),array('personal','addEvent','edit'))){ ?><span class="triangle"></span><?php } ?>
          <?php if(in_array((ACTION_NAME),array('eventDetail'))&& $uid==$mid){ ?><span class="triangle"></span><?php } ?>
        </a></li>
			</ul>
		</div>
...
<div class="page_tit">
  ...
  <div class="right_box box_create"><a href="<?php echo U('/Index/addEvent');?>" class="btn-create">发起活动</a></div>
 <!--发起者&关注者-->
    ...
    <div class="right_box">
            <div>
              <h2>发起者</h2>
              <ul class="ListBox">
                <li><span class="headpic50"><?php echo getUserSpace($uid,'fn','','{uavatar}') ?></span><span class="user-grade"><?php echo getUserSpace($uid,'fn','','{uname}') ?></span></li>
              </ul>
              <div class="btm"></div>
            </div>
            <div>
              <h2>关注者</h2>
              <ul class="ListBox">
                <?php if(is_array($attention)): ?><?php $i = 0;?><?php $__LIST__ = $attention?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><span class="headpic50"><?php echo getUserSpace($vo["uid"],'','','{avatar}') ?></span><span class="user-grade"><?php echo getUserSpace($vo["uid"],'','','{uname}') ?></span></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                <div class="c"></div>
              </ul>
 ...
 <!--推荐活动--> 
  ...
  <!--推荐活动END-->
        ...
        <div class="tab-menu">
          ...
          <ul>
            <li class="current"><a href="<?php echo U('//eventDetail',array('id'=>$id,'uid'=>$uid));?>"><span>活动</span></a></li>
            <li><a href="<?php echo U('//member',array('id'=>$id));?>"><span>成员</span></a></li>
          </ul>
        </div>
        <!-- 切换标签 end  -->
        <div class="groupBox hide">
          <div class="boxL">
            <div class="box1">
              <div class="info">
                <div class="img"> <img src="<?php echo ($cover); ?>" /> </div>
                <div class="list">
                  <ul>
                    <li>
                      <div class="c1">发起者：</div>
                      <div class="c2"><?php echo ($user_info[$uid]['space_link']); ?></div>
                    </li>
                    <li>
                      <div class="c1"> 活动分类：</div>
                      <div class="c2"> <?php echo ($type); ?> </div>
                    </li>
                    <li>
                    ...
                    <li>
                      ...
                      <div class="c2"> <?php echo ($costExplain); ?> </div>
                    </li>
                    <li>
                      <div class="c1"> 活动城市：</div>
                      <div class="c2"> <?php echo ($city); ?> </div>
                    </li>
                    <li>
                    ...
                    <li>
                      ...
                      <div class="c2"> <?php echo ($time); ?> </div>
                    </li>
                    <li>
                      <div class="c1"> 截止报名：</div>
                      <div class="c2"> <?php echo ($dl); ?> </div>
                    </li>
                    <li>
                      <div class="c1"> 剩余名额：</div>
                      <div class="c2"> <?php echo ($lc); ?> </div>
                    </li>
                    <li>
                      <div class="c1"> 联系方式：</div>
                      <div class="c2"> <?php echo ($contact); ?> </div>
                    </li>
                    <li>
                    ...
                    <li>参加人数（<a href="<?php echo U('//member', array('id'=>$id,'action'=>'join'));?>"><?php echo ($joinCount); ?></a>） 关注人数（<a href="<?php echo U('//member', array('id'=>$id,'action'=>'att'));?>"><?php echo ($attentionCount); ?></a>）<?php if($verifyCount): ?>待审人数（<a href="<?php echo U('//member', array('id'=>$id,'action'=>'verify'));?>"><?php echo ($verifyCount); ?></a>）<?php endif; ?></li>
                    <?php if( $eTime>time()){ ?>
                    <li> <span class="cGreen lh35" id="event_satus">此活动正在进行中……</span> </li>
                    <li>
                      <?php if( $admin ){ ?>
                      <span id="event_edit_button">
                      <button class="btn_b" style="margin-right:5px;" onclick="javascript:endEvent( '<?php echo ($id); ?>' )">提前结束</button>
                      <button class="btn_b" style="margin-right:5px;" onclick="javascript:location.href='<?php echo U('/Index/edit',array('id'=>$id));?>';">编辑活动</button>
     				  <button class="btn_b" style="margin-right:5px;" onclick="javascript:delEvent( '<?php echo ($id); ?>' ,true)">删除活动</button>
                      </span>
                      <?php } ?>
                      <span class="detail_joinIn_<?php echo ($id); ?>">
                      <?php if( $follow ){ ?>
                      <?php if( $canJoin ){ ?>
                      <?php if( $limitCount>0 ) { ?>
                      <button class="btn_b" style="margin-right:5px;"  onclick="javascript:EventAction( <?php echo ($id); ?>,<?php echo ($opts['allow']); ?>,'joinIn')">我要参加</button>
                      <?php }else{ ?>
                      <span class="cGreen lh35">名额已满</span>
                      <?php } ?>
                      <?php }else if( false == $admin ){ ?>
                      <?php if( $opts['allow'] == 1 && $hasMember != 1){ ?>
                      <span class="cGreen lh35">已提交申请,等待审核中,<button class="btn_w" style="margin-right:5px;" onclick="javascript:EventDelAction(<?php echo ($id); ?>,<?php echo ($opts['allow']); ?>,'joinIn')">取消申请</button></span>
                      <?php }else{ ?>
                      <button class="btn_w" style="margin-right:5px;"  onclick="javascript:EventDelAction( <?php echo ($id); ?>,<?php echo ($opts['allow']); ?>,'joinIn')">取消参加</button>
                      <?php } ?>
                      <?php } ?>
                      <?php } ?>
                      <?php if( $canAtt ){ ?>
                            <span class="detail_attention_<?php echo ($id); ?>">
                            <button class="btn_b"  style="margin-right:5px;"  onclick="javascript:EventAction( <?php echo ($id); ?>,null,'attention')">我要关注</button>
                            </span>
                      <?php }else if( false == $admin && true == $canJoin){ ?>
                            <span class="detail_attention_<?php echo ($id); ?>">
                            <button class="btn_w"  style="margin-right:5px;"  onclick="javascript:EventDelAction( <?php echo ($id); ?>,null,'attention')">取消关注</button>
                            </span>
                      <?php } ?>
                        
                      </span>
                      <span id="share_button">
                      <?php echo W('ShareToFeed',array('title'=>'分享活动','initHTML'=>urlencode('我分享了一个活动“'.$title.'”'),'url'=>U('event/Index/eventDetail',array('id'=>$id,'uid'=>$uid)),'attachId'=>$coverId,'from'=>'event'));?>
                      </span>
                    </li>
                    <?php }else { ?>
                    <li>
            ...
            <div class="box1">
              ...
              <div class="lh180" style="word-wrap:break-word;"><?php echo ($explain); ?></div>
            </div>
            <div class="box1">
              <h3><span class="right f12px"><a href="<?php echo U('//member', array('id'=>$id,'action'=>'join'));?>">查看全部>></a></span><strong>活动成员</strong>(共<?php echo ($joinCount); ?>人)</h3>
              <ul class="pic50box">
                <?php if(is_array($member)): ?><?php $i = 0;?><?php $__LIST__ = array_slice($member,0,8) ?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><span class="headpic50"><?php echo getUserSpace($vo["uid"],'','','{avatar}') ?></span><span class="user-grade"><?php echo getUserSpace($vo["uid"],'','','{uname}') ?></span></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                <div class="c"></div>
              </ul>
            </div>
            <div class="feed_list">
                <div class="repeat">
                   <?php echo W('Comment',array('tpl'=>'detail', 'table'=>'event', 'row_id'=>$id, 'canrepost'=>'0', 'cancomment'=>1, 'cancomment_old'=>0, 'showlist'=>1, 'limit'=>'20', 'order'=>'DESC'));?>
                </div>
            </div>
            </div>
            <div class="c"></div>
