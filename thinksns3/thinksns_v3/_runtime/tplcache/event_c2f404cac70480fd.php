...
<!--搜索活动&分类浏览END-->   
 ...
 <!--推荐活动--> 
  ...
  <!--推荐活动END-->
      ...
      <div class="mainbox_appC">
        <div class="tab-menu">
          <!-- 切换标签 begin  -->
          <ul>
            <li <?php if(!in_array($_GET['action'],array('join','att'))){echo 'class="current"';} ?>><a href="<?php echo U('//personal', array('action'=>'launch'));?>" ><span><?php echo ($name); ?>发起的活动</span></a></li>
            <li <?php if($_GET['action'] == 'join'): ?>class="current"<?php endif; ?>><a href="<?php echo U('//personal', array('action'=>'join'));?>" ><span><?php echo ($name); ?>参与的活动</span></a></li>
            <li <?php if($_GET['action'] == 'att'): ?>class="current"<?php endif; ?>><a href="<?php echo U('//personal', array('action'=>'att'));?>" ><span><?php echo ($name); ?>关注的活动</span></a></li>
          </ul>
        </div>
        <!-- 切换标签 end  -->
        <div class="groupBox">
  ...
  <div class="LogList">
    <ul class="search">
      <?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="lineD_btm" id="event_<?php echo ($vo['id']); ?>">
          <div class="left"><span><a href="<?php echo U('event/Index/eventDetail', array('id'=>$vo['id'], 'uid'=>$vo['uid']));?>"><img src="<?php echo ($vo['cover']); ?>" width="100" height="100"/></a></span></div>
          <div class="right rmenu" style="width: 60px;">
            <?php if( $vo['eTime']>time()){ ?>                    
              <span class="cGreen lh35" id="event_satus_<?php echo ($vo['id']); ?>">                   
	            <?php if( $vo['admin'] ){ ?>
              <a href="javascript:endEvent( '<?php echo ($vo['id']); ?>' )">提前结束</a>
              <a href="javascript:location.href='<?php echo U('event/Index/edit',array('id'=>$vo['id']));?>';">编辑活动</a>
              <a href="javascript:delEvent( '<?php echo ($vo['id']); ?>' )">删除活动</a>
                <?php } ?>                    
              </span>
              <span class="list_joinIn_<?php echo ($vo['id']); ?>">
              <?php if( $vo['follow'] ){ ?>
              <?php if( $vo['canJoin'] ){ ?>
                  <?php if( $vo['limitCount'] >0 ){ ?>
                    <a href="javascript:EventAction( <?php echo ($vo['id']); ?>,<?php echo ($vo['opts']['allow']); ?>,'joinIn')">我要参加</a>
                  <?php }else{ ?>
                  <span class="cGreen lh35 pl5">名额已满</span>
                <?php } ?>
            <?php }else if( false == $vo['admin'] ){ ?>
                    <?php if( $vo['opts']['allow'] == 1 && $vo['hasMember'] != 1){ ?>
                       <a href="javascript:EventDelAction( <?php echo ($vo['id']); ?>,<?php echo ($vo['opts']['allow']); ?>,'joinIn' )">取消申请</a>
                  <?php }else{ ?>
                       <a href="javascript:EventDelAction( <?php echo ($vo['id']); ?>,null,'joinIn' )">取消参加</a>
                  <?php } ?>
              <?php } ?>
              <?php } ?>
            <?php if( $vo['canAtt'] ){ ?>
                    <span class="list_attention_<?php echo ($vo['id']); ?>">
                    <a href="javascript:EventAction( <?php echo ($vo['id']); ?>,null,'attention')">我要关注</a>
                    </span>
            <?php }else if( false == $vo['admin'] && true == $vo['canJoin']){ ?>
                    <span class="list_attention_<?php echo ($vo['id']); ?>">
                    <a href="javascript:EventDelAction( <?php echo ($vo['id']); ?>,null,'attention')">取消关注</a>
                    </span>
            <?php } ?>
              </span>
            <?php }else{ ?>
              <span class="cGreen lh35 pl5">活动结束</span>
            <?php } ?>
          </div>
          <div style="margin:0 70px 0 115px;">
            <h3><a href="<?php echo U('event/Index/eventDetail', array('id'=>$vo['id'], 'uid'=>$vo['uid']));?>"><?php echo ($vo['title']); ?></a> <span class="cGray2">[<?php echo ($vo['type']); ?>]</span></h3>
            <p><span class="cGray2">活动时间：</span>[<?php echo ($vo['time']); ?>] </p>
            <p><span class="cGray2">活动地点：</span><?php echo ($vo['address']); ?> <?php echo ($vo['area']); ?></p>
            <p><span class="cGray2">发起人：</span><?php echo ($vo['user_info']['space_link']); ?></p>
            <?php if( false == empty( $vo['users'] ) ){ ?>
            ...
            <?php $vo['users']; ?>
              ...
              foreach( $vo['users'] as $so ) {
                $_i++;
                if($_i++ <17){ ?>
              <a href="<?php echo U('public/Profile/',array('uid'=>$so['uid']));?>" ><?php echo (getUserName($so['uid'])); ?></a> &nbsp;
              <?php }} ?>
              <?php if(count($vo['users']) > 8){ ?>
              <a href="<?php echo U('//member', array('id'=>$vo['id'],'action'=>'join'));?>">更多>></a>
              <?php } ?>
            </p>-->
            <?php } ?>
            <p><span class=" mr10"><?php if($vo['opts']['allow'] && $vo['uid'] == $mid) echo "待审人数（".$vo['verifyCount']."）";?> 参加人数（<?php echo ($vo['joinCount']); ?>） 关注人数（<?php echo ($vo['attentionCount']); ?>）</span></p>
          </div>
        </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
    </ul>
    <div id="Pagination" class="page"><?php echo ($html); ?></div>
  </div>
</div>
          </div>
          <!-- 活动 end  -->
