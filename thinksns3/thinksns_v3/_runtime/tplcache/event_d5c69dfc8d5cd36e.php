...
<div class="page_tit">
  ...
  <div class="main no_l">
    <!-- 画布 begin  -->
    <div class="mainbox clearfix"> <div class="mainbox_appR">
  ...
  <!--推荐活动END-->
      ...
      <div class="mainbox_appC">
        ...
        <div class="tab-menu">
          <!-- 切换标签 begin  -->
          <ul>
            <li><a href="<?php echo U('/Index/eventDetail',array('id'=>$id,'uid'=>$uid));?>"><span>活动</span></a></li>
            <li class="current"><a href="<?php echo U('//member',array('id'=>$id));?>"><span>成员</span></a></li>
            <!-- <li><a href="__URL__/photos/id/<?php echo ($id); ?>/uid/<?php echo ($uid); ?>"><span>照片</span></a></li> -->
          </ul>
        </div>
        <!-- 切换标签 end  -->
        <div class="MenuSub"> <a href="<?php echo U('//member',array('id'=>$id));?>" 
          <?php if (!in_array($_GET['action'],array('join','att','verify'))) {echo "class='fB'";}else{echo "class='cGray'";} ?>
          >全部成员</a>┊ <a href="<?php echo U('//member',array('id'=>$id,'action'=>'join'));?>" 
          <?php if (isset( $_GET['action'] ) && 'join' == $_GET['action']) {echo "class='fB'";}else{echo "class='cGray'";} ?>
          >（<?php echo ($joinCount); ?>）人参加</a>┊ <a href="<?php echo U('//member',array('id'=>$id,'action'=>'att'));?>" 
          <?php if (isset( $_GET['action'] ) && 'att' == $_GET['action']) {echo "class='fB'";}else{echo "class='cGray'";} ?>
          >（<?php echo ($attentionCount); ?>）人关注</a>
          <?php if( $uid == $mid ){ ?>
          ┊ <a href="<?php echo U('//member',array('id'=>$id,'action'=>'verify'));?>" 
          <?php if (isset( $_GET['action'] ) && 'verify' == $_GET['action']) {echo "class='fB'";}else{echo "class='cGray'";} ?>
          >（<?php echo ($verifyCount); ?>）人待审核</a>
          <?php }?>
        </div>
        <div class="flist" style="overflow:hidden;">
          <ul>
            <?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="event_user_<?php echo ($vo['id']); ?> lineD_btm" style="overflow:hidden;_zoom:1" >
                <div class="left" style="width: 60px;"><span class="headpic50"><?php echo getUserSpace($vo["uid"],'fn','','{uavatar}') ?></span></div>
                <div style=" margin-left:60px; height:50px;">
                  <h3>
                    <div class="right fn f12px">
                      <?php if( 'admin' != $vo['action'] && $uid == $mid  ){ ?>
                      <a href="javascript:adminDelAction( <?php echo ($id); ?>,<?php echo ($vo['uid']); ?>,'<?php echo ($vo['action']); ?>',<?php echo ($optsId); ?>);">删除成员</a><br />
                      <?php } ?>
                      <?php if( isset( $verifyCount ) && $vo['status'] == 0 ){ ?>
                      <a href="javascript:agree( <?php echo ($vo['id']); ?>,<?php echo ($id); ?>,<?php echo ($uid); ?>,<?php echo ($vo['uid']); ?>);">同意申请</a>
                      <?php }?>
                    </div>
                    <?php echo getUserSpace($vo["uid"],'','','{uname}') ?><span class="f12px fn cRed">（<?php echo ($vo['role']); ?>）</span></h3>
