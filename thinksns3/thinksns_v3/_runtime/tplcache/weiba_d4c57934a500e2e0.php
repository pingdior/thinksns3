              ...
              <?php } ?>
                    ...
                    <div class="right-wrap">
                       <div class="person-info mb20 clearfix border-b">
                           <dl class="person-info-t clearfix">
                              <dt><img src="<?php echo ($weiba_detail['logo']); ?>" /></dt>
                              <dd>
                                <p class="mb10">
                                  <?php echo ($weiba_detail['weiba_name']); ?>&nbsp;<?php if(in_array($mid,$weiba_admin_uids) || CheckPermission('core_admin','admin_login')){ ?>[<a href="<?php echo U('weiba/Manage/index',array('weiba_id'=>$weiba_detail['weiba_id']));?>">管理</a>]<?php } ?>
                                </p>
                                <p class="mb10">粉丝数：<?php echo (($weiba_detail['follower_count'])?($weiba_detail['follower_count']):0); ?></p>
                                <p class="mb10">帖子数：<?php echo (($weiba_detail['thread_count'])?($weiba_detail['thread_count']):0); ?></p>
                                <p class="mb10">
                                <?php if($follow_state[$weiba_detail['weiba_id']]['following'] && !in_array($mid,$weiba_admin_uids)){ ?>
	                                <?php if(CheckPermission('weiba_normal','weiba_apply_manage')){ ?>
		                                <?php if($weiba_super_admin){ ?><a onclick="weiba_apply(<?php echo ($weiba_detail['weiba_id']); ?>,2)" href="javascript:void(0)">申请小吧</a>
		                                <?php }else{ ?>
		                                <a onclick="weiba_apply(<?php echo ($weiba_detail['weiba_id']); ?>,3)" href="javascript:void(0)">申请吧主</a>
		                                <?php } ?>
	                                <?php } ?>
                                <?php } ?></p>
                                <p><?php echo W('FollowWeiba',array('weiba_id'=>$weiba_detail['weiba_id'],'follow_state'=>array('following'=>$follow_state[$weiba_detail['weiba_id']]['following']),'isrefresh'=>1));?></p>
                              </dd>
                           </dl>
                       </div>
                       <div class="attention mb20 clearfix border-b">
                         <h3>微吧简介</h3>
                         <div class="mb20 content"><p><?php echo ($weiba_detail['intro']); ?></p></div>
                       </div>
                       <div class="attention mb20 clearfix border-b">
                         <h3>公告</h3>
                         <div class="mb20 content"><p><?php echo ($weiba_detail['notify']); ?></p></div>
                       </div>
                       <div class="attention mb20 clearfix border-b">
                         <h3>吧主<span>(<?php echo (($weiba_admin_count)?($weiba_admin_count):0); ?>人)</span></h3>
                         <?php if(is_array($weiba_admin)): ?><?php $i = 0;?><?php $__LIST__ = $weiba_admin?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dl>
                             <dt><a href="<?php echo ($user_info[$vo['follower_uid']]['space_url']); ?>" target="_blank"><img src="<?php echo ($user_info[$vo['follower_uid']]['avatar_small']); ?>" event-node="face_card" uid="<?php echo ($vo["follower_uid"]); ?>"/></a></dt>
                             <dd>
                                <div class="right"><?php if($vo['follower_uid']!=$mid){ ?><?php echo W('FollowBtn', array('fid'=>$vo['follower_uid'], 'uname'=>$user_info[$vo['follower_uid']]['uname'], 'follow_state'=>$follow_user_state[$vo['follower_uid']], 'type'=>'normal'));?><?php } ?></div>
                                <p>
                                  <?php echo ($user_info[$vo['follower_uid']]['space_link']); ?>
                                ...
                                <p><?php if($vo['level']==3){ ?>超级吧主<?php }else{ ?>小吧<?php } ?></p></dd>
                          </dl><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?> 
                       </div>

                       <?php echo W('FollowWeibaList',array('follower_uid'=>$mid));?> 
                 ...
                 <div id="col5" class="st-index-main">
                    <div class="extend-foot">
                        <div class="tab-menu">
                        <?php if($GLOBALS['ts']['mid'] && CheckPermission('weiba_normal','weiba_post') ){ ?>
                            <a href="javascript:void(0)" onclick="check_post(<?php echo ($weiba_detail['weiba_id']); ?>,<?php echo ($weiba_detail['who_can_post']); ?>)" class="btn-green-small right"><span>发帖子</span></a>
                            <?php } ?>
                            <ul>
                                <li <?php if($type=='all'){ ?>class="current"<?php } ?>><span><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$weiba_detail['weiba_id']));?>">全部</a></span></li>
                                <li <?php if($type=='digest'){ ?>class="current"<?php } ?>><span><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$weiba_detail['weiba_id'],'type'=>'digest'));?>">精华</a></span></li>
                            </ul>
                        </div>
                        <div class="weiba-sort f9 clearfix">
                            <span class="right">排序方式：<a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$weiba_detail['weiba_id'],'type'=>$type,'order'=>'reply_time'));?>" <?php if($order=='reply_time'){ ?>class="current"<?php } ?>>回复时间</a><i class="vline">|</i><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$weiba_detail['weiba_id'],'type'=>$type,'order'=>'post_time'));?>" <?php if($order=='post_time'){ ?>class="current"<?php } ?>>发帖时间</a></span><span>共有<?php echo (($post_count)?($post_count):0); ?>个帖子</span>
                        </div>
                           ...
                           <ul>
                             <?php if(is_array($list["data"])): ?><?php $i = 0;?><?php $__LIST__ = $list["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li>
                                <div class="name">
                                  ...
                                  <a href="<?php echo U('weiba/Index/postDetail',array('post_id'=>$vo['post_id']));?>"><span style="vertical-align:-2px;"><?php echo ($vo["title"]); ?></span><?php if(strstr($vo['content'],'<img')){ ?><i class="ico-img"></i><?php } ?><?php if($vo['top']==2){ ?><i class="ico-bar-top ml5">置顶</i><?php } ?><?php if($vo['top']==1){ ?><i class="ico-bar-top0 ml5">置顶</i><?php } ?><?php if($vo['digest']==1){ ?><i class="ico-bar-fine ml5">精华</i><?php } ?></a>
                                </div>
                                <div class="f9">
                                  <span class="right"><?php if($vo['last_reply_time'] && $vo['last_reply_uid']){ ?><?php echo ($user_info[$vo['last_reply_uid']]['space_link']); ?><span class="pl5 pr5">/</span><?php echo (friendlyDate($vo["last_reply_time"])); ?><?php } ?></span><span><?php echo ($user_info[$vo['post_uid']]['space_link']); ?><span class="pl5 pr5">/</span><?php echo (friendlyDate($vo["post_time"])); ?>发布</span>
                                </div>
                              </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                            
                          </ul>
                          <div id="page" class="page" style=""><?php echo ($list["html"]); ?></div>
                        </div>
                     </div>
                 </div>
           	 </div>
