...
class UserDataModel extends Model {
	 ...
	 * @return void
	 */
	public function  getEventCountById($uid = ''){
		if(empty($uid)) {
			$uid = $this->uid;
		}
		$EventCount=0;
		$sql = 'select count(*) eventCount from '.C('DB_PREFIX').'event where uid='.$uid;
		$list = M()->query($sql);
		foreach ($list as $vo){
			$eventCount=intval($vo['eventCount']);
		}
		return $eventCount;
	}
	 ...
	 * @return void
	 */
	public function  getJoinCountById($uid = ''){
		if(empty($uid)) {
			$uid = $this->uid;
		}
		$joinCount=0;
		$sql = "select count(*) joinCount from ".C('DB_PREFIX')."event_user where action='joinIn' and status=1 and uid=".$uid;
		$list = M()->query($sql);
		foreach ($list as $vo){
			$joinCount=intval($vo['joinCount']);
		}
		return $joinCount;
	}
	/**
